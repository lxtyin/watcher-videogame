...
- Added a shared gameplay-mode / map registry in `packages/shared/src/content/maps.ts`.
  - `free_default` stays bound to free mode and keeps debug tools enabled
  - `race_sprint` is the new race-mode map and disables debug tools
- Added new board tiles:
  - `start`
  - `goal`
- Extended shared snapshots and Colyseus schema with gameplay metadata:
  - `mapId`
  - `mapLabel`
  - `mode`
  - `allowDebugTools`
...
- Added race finishing flow to both:
  - `packages/shared/src/simulation/engine.ts`
  - `packages/server/src/rooms/WatcherRoom.ts`
  - reaching `goal` now records rank / arrival turn
  - finished players are skipped by later turn rotation
  - the match enters settlement once all players are finished
- Updated server matchmaking so `mapId` creates separate room pools through `filterBy(["mapId"])`.
- Updated client flow:
  - `useWatcherConnection.ts` now reads `?map=...`
  - `HudSidebar.tsx` now exposes a minimal free/race map switcher
  - race maps hide the debug grant UI
  - `App.tsx` now mounts a race settlement overlay
  - settlement rows show pet thumbnail, id/name, rank, and arrival turn
- Added race-mode tile visuals and inspection copy:
  - `start`
  - `goal`
- Added race golden coverage in `packages/shared/src/goldens/cases/race.ts`:
  - active player reaches goal and the turn passes
  - last unfinished player reaches goal and settlement starts
- Updated docs:
  - `docs/index.md`
  - `docs/arch/共享规则层.md`
  - `docs/arch/前后端联机原型.md`
- Validation:
  ...
  - `npm.cmd run build`
  - browser text smoke:
    - `output/web-game/race-mode-text-smoke/state.json`
    - confirmed:
      - `mode = race`
      - `mapId = race_sprint`
...
## 2026-04-04 Mobile Room Adaptation

- Updated the in-room client shell to support phone landscape play without moving the sidebar above the scene.
  - the room layout now keeps the sidebar on the left across desktop and mobile
  - added a shared collapse toggle so the sidebar can be hidden on both PC and mobile
- Added a portrait-only mobile overlay that asks the player to rotate the device before playing.
- Updated camera input mapping in `GameBoardCanvas`.
  - single touch no longer rotates the camera
  - two-finger touch now maps to the same rotate/zoom camera control path as mouse middle + wheel
  - single-touch scene interaction still stays available for gameplay actions
- Verified:
  - `npm.cmd run typecheck`
  - `npm.cmd run build --workspace @watcher/client`
- Known non-blocking issue remains unchanged:
  - root `npm.cmd run build` still hits the existing server `tsup --dts` declaration issue when launched from the current path
